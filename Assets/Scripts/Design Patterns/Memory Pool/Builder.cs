...
{
	public enum ObjectsState { Alive, Dead, Both};
	...
	public class Builder : PropertyAttribute
	{
		...
		public void build(bool p_isOnNetwork = false)
		{
		...
		/// <summary>
		/// Gets all objects. According to states 0 dead 1 Alive 2 Both
		/// </summary>
		/// <returns>The all objects.</returns>
		/// <param name="p_state">If set to <c>true</c> p_state.</param>
		public List<GameObject> GetAllObjs(ObjectsState p_state)
		{
			List<GameObject> buffer = null;

			foreach(GameObject obj in m_objBank) {
				if(p_state == ObjectsState.Both) {
					buffer.Add(obj);
				} else if(p_state == ObjectsState.Dead) {
					if(obj.activeSelf == false) {
						buffer.Add(obj);
					}
				} else {
